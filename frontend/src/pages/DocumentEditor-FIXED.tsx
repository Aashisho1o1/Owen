...
const DocumentEditor: React.FC = () => {
  ...
  // 🔥 FIXED: Load document on mount - ONLY depends on documentId
  useEffect(() => {
  ...
  }, [documentId]); // ✅ ONLY documentId - no function dependencies!

  // 🔥 FIXED: Update document content - ONLY depends on specific values
  useEffect(() => {
    if (currentDocument && editorContent !== currentDocument.content) {
      console.log('📝 Updating content...');
      updateContent(editorContent);
    }
  }, [editorContent, currentDocument?.content, currentDocument?.id]); // ✅ Specific values only

  // 🔥 FIXED: Update document title - ONLY depends on specific values
  useEffect(() => {
    if (currentDocument && documentTitle !== currentDocument.title) {
      console.log('🏷️ Updating title...');
      updateTitle(documentTitle);
    }
  }, [documentTitle, currentDocument?.title, currentDocument?.id]); // ✅ Specific values only
